Point of Sale Software
======================

- [Trello Board](https://trello.com/b/1pn6hNrT/my-trello-board)
- [Ruby on Rails Guides](https://guides.rubyonrails.org/index.html)

# How To Run

- [Install Ruby on Rails](https://guides.rubyonrails.org/install_ruby_on_rails.html)
- Run `bundle install` to install dependencies

## Development Environment

Run `bin/dev`. This will run the development server on [http://localhost:3000](http://localhost:3000) / [http://127.0.0.1:3000](http://127.0.0.1:3000).

## Database Migrations

Run `bin/rails db:migrate`.

## Create an account

To create an account run `bin/rails c` to get to the console. Then run (replace the values if you wish):

```ruby
User.create(name: "Demo", email_address: "example@example.com", password: "example", admin: true)
```
